...
test("therapist signup → DB record → admin approval", async ({ page }) => {
  ...
  // ── Step 3: Who You See ──────────────────────────────────────────
  // "individual" is pre-selected in state defaults — verify it's active,
  // do NOT click it (clicking would toggle it OFF).
  await expect(page.getByTestId("signup-client-type-individual")).toHaveClass(
    /bg-\[#2D4A3E\]/,
    { timeout: 3_000 },
  );

  // Age groups — pick adult (starts empty, so clicking adds it)
