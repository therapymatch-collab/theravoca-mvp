        ...
        >
          ...
          {editTherapist && (
            <div className="space-y-4 mt-2">
              ...
              {/* ── Reliability & T6 (read-only) ── */}
              {(editTherapist.reliability || editTherapist.t6_session_expectations) && (
                <div className="border-t border-[#E8E5DF] pt-4 space-y-3">
                  <h4 className="text-sm font-semibold text-[#2D4A3E]">v5 Scoring Data (read-only)</h4>
                  {editTherapist.t6_session_expectations?.length > 0 && (
                    <FieldRow label="Session expectations (T6)">
                      <div className="flex gap-1.5 flex-wrap">
                        {editTherapist.t6_session_expectations.map((e) => (
                          <span key={e} className="px-2 py-0.5 bg-[#2D4A3E]/10 text-[#2D4A3E] rounded text-xs">
                            {e.replace(/_/g, " ")}
                          </span>
                        ))}
                      </div>
                  ...
                  {editTherapist.t6b_early_sessions && (
                    <FieldRow label="Early sessions approach (T6b)">
                      <p className="text-sm text-gray-600 bg-[#FDFBF7] p-2 rounded border border-[#E8E5DF]">
                        {editTherapist.t6b_early_sessions}
                      </p>
                    </FieldRow>
                  )}
                  {editTherapist.reliability && (
                    <FieldRow label="Reliability scores">
                      <div className="space-y-1.5">
                        {Object.entries(editTherapist.reliability)
                          .filter(([k]) => !k.startsWith("_") && k !== "last_feedback_at")
                          .map(([k, v]) => {
                            ...
                            return (
                              <div key={k} className="flex items-center gap-2 text-xs">
                                <span className="w-40 text-gray-500 capitalize">{k.replace(/_/g, " ")}</span>
                        ...
                        {editTherapist.reliability.last_feedback_at && (
                          <p className="text-xs text-gray-400 mt-1">
                            Last updated: {new Date(editTherapist.reliability.last_feedback_at).toLocaleDateString()}
                          </p>
                        )}
                      </div>
                    </FieldRow>
