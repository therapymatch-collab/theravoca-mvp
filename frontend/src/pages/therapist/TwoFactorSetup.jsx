    ...
    <div className="min-h-screen bg-[#FDFBF7] flex flex-col">
      ...
      <main className="flex-1 px-5 py-12 md:py-16">
        <div className="max-w-2xl mx-auto" data-testid="therapist-2fa-page">
          ...
          <DeleteAccountPanel sessionEmail={session?.email || ""} />
        </div>
      </main>
      <Footer />
    </div>
...
// Self-serve account-deletion panel. Two-step confirmation:
// (1) type your email exactly, (2) type DELETE in caps. Both must
// match before the destructive POST fires. Backend also re-checks.
function DeleteAccountPanel({ sessionEmail }) {
  const navigate = useNavigate();
  ...
  const onDelete = async () => {
    if (!canDelete) return;
    if (!window.confirm(
      "This permanently deletes your account, profile, and any active subscription. This cannot be undone after 24 hours. Continue?"
    )) return;
    setBusy(true);
    try {
      await sessionClient().post("/portal/therapist/delete-account", {
        confirm_email: confirmEmail.trim(),
        confirm_phrase: confirmPhrase,
      });
      toast.success("Account deleted. You'll be signed out.");
      // Clear session + bounce to landing.
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-[#5C2620] mt-2 leading-relaxed">
        Permanently removes your profile, contact details, license
        document, and login. Cancels any active TheraVoca subscription
        at end-of-period (no refund mid-cycle, no surprise renewals).
        We retain anonymized aggregate analytics + records we're
        legally required to keep. <strong>Reversible within 24 hours
        if you misclick</strong> -- email{" "}
