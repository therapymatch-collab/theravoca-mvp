        ...
        <div className="space-y-4">
          <div className="flex flex-wrap gap-4">
            <div className="px-4 py-3 bg-[#F5F3EF] rounded-lg">
              <div className="text-2xl font-bold text-[#2E4057]">{result.total}</div>
                ...
                <tbody>
                  {(result.candidates || []).map((c, i) => (
                    <tr key={i} className={`border-t border-[#E8E5DF] ${i % 2 === 0 ? "bg-white" : "bg-[#FAFAF8]"}`}>
                      ...
                      <td className="px-3 py-2">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${c.source === "psychology_today" ? "bg-purple-100 text-purple-700" : c.source === "google_maps" ? "bg-blue-100 text-blue-700" : c.source === "therapyden" ? "bg-green-100 text-green-700" : c.source === "goodtherapy" ? "bg-orange-100 text-orange-700" : "bg-gray-100 text-gray-700"}`}>
                          {(c.source || "unknown").replace(/_/g, " ")}
                        </span>
                      </td>
                      <td className="px-3 py-2">
                        {c.profile_url ? (<a href={c.profile_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline"><ExternalLink size={14} /></a>) : "-"}
                      </td>
                    </tr>
                  ))}
                </tbody>
